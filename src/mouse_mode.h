...
static const uint32 INVALID_TILE_INDEX = 0xFFFFu; ///< Invalid selector tile index.
...
public:
	...
	/**
	 * Get the ride data of a voxel if available, for rendering.
	 * @param voxel %Voxel being rendered.
	 * @param voxel_pos Position of the voxel in the world.
	 * @param [out] sri Ride instance that should be rendered if \c true is returned.
	 * @param [out] instance_data Instance data that should be rendered if \c true is returned.
	 * @return \c True if the data supplied through \a sri and \a instance_data should be used, else \c false.
	 */
...
public:
	...
	void MarkDirty() override
	{
		for (int x = 0; x < this->area.width; x++) {
			...
			for (int y = 0; y < this->area.height; y++) {
				...
				if (!td.cursor_enabled || td.cursor_height < 0) continue;

				MarkVoxelDirty(XYZPoint16(xpos, ypos, td.cursor_height), 0);
			}
		}
	}
	...
	uint32 GetRange(uint xpos, uint ypos) override
	{
		return 0;
	}
...
struct RideTileData : public CursorTileData {
	 ...
	 * @see TileDataMouseMode::GetRange
	 */
